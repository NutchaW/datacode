...
SIRS_EAKF_model <- R6Class(
  ...
  private = list(
    ...
    .fitData = NULL, # fitted stae variables at the end of num_times
    ...
    .steps = integer(0), # number of steps that predict
  ...
  public = list(
    fit = function(data, AbsHumidity, phi) {
      ...
      obs <- ILIp[temp[1]:length(ILIp)] # observations
      private$.obs <- obs
      startdate <- obstime[temp[1]]
      s <- paste0("01/01/", private$.year) # the first day of the year
      private$.ts <- as.numeric(as.Date(startdate, format = "%m/%d/%y")) -
    ...
    forecast = function(steps) {
      ...
      private$.steps <- steps # number of steps we want to predict
      xprior <- replicate(private$.steps, matrix(NaN, 7, private$.num_ens))
      xpost <- xprior
     
      # output prediction results of EAKFC
      Tpkpred <- rep(0, private$.steps)
      ...
      for (tt in 1:private$.steps) {
        ave <- private$.obs[max(1, tt - 1 + private$.num_times)]
        ave <- ave + private$.obs[max(1, tt - 2 + private$.num_times)]
        ave <- ave + private$.obs[max(1, tt - 3 + private$.num_times)]
        ave <- ave / 3
        obs_var <- 10^5 + ave^2 / 5
        # inflation of x before assimilation
        x <- as.matrix(apply(x, 1, mean)) %*% rep(1, private$.num_ens) +
        ...
        prior_mean <- mean(H %*% x)
        post_mean <- post_var * (prior_mean / prior_var + private$.obs[tt + private$.num_times] / obs_var)
        # Compute alpha and adjust distribution to conform to posterior moments
        alpha <- (obs_var / (obs_var + prior_var))^0.5
        dy <- post_mean + alpha * ((H %*% x) - prior_mean) - H %*% x
        #  Loop over each state variable
        ...
        # EAKF update is finished
        xpred <- replicate(private$.steps, matrix(0, 7, private$.num_ens))
        # assign xpred before tt as posterior
        for (t in 1:tt) {
          xpred[, , t] <- xpost[, , t]
          xpred[3, , t] <- private$.obs[t] * rep(1, private$.num_ens) # assign as true observations
        }
...
SIRS_EAKF_model$initialize()

### fit
SIRS_EAKF_model$fit(data, AbsHumidity, phi)

### forecast
steps <- 3 # forecast ahead `step` number of weeks
SIRS_EAKF_model$forecast(steps = steps)
